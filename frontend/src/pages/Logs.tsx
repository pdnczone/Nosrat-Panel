...
const Logs = () => {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-foreground mb-2">{t.logs.title}</h1>
        <p className="text-muted-foreground">{t.logs.subtitle}</p>
      </div>

      <div 
        ref={logContainerRef}
        className="bg-slate-950 rounded-xl border border-border/60 shadow-glass-dark p-6 font-mono text-sm overflow-auto" 
      ...
      >
        {logs.length === 0 ? (
          ...
          logs.map((log, index) => (
            <div key={index} className="mb-1 hover:bg-white/5 px-2 py-1 rounded">
