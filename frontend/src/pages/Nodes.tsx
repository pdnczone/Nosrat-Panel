...
import { Plus, Copy, Download, Server, ChevronDown, Check } from 'lucide-react'
...
import { fetchCACertificate, downloadCACertificate, registerNode } from '../services/nodesApi'

const Nodes = () => {
  const { t } = useLanguage()
  const { show } = useToast()
  const { nodes, loading, error, errorDetail, refresh } = useNodes('iran')
  ...
  const openCA = async () => {
    ...
    setCertLoading(true)
    try {
      const text = await fetchCACertificate()
  ...
  return (
    <div className="w-full max-w-7xl mx-auto">
      <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-3xl font-bold text-foreground">{t.nodes.title}</h1>
          <p className="mt-1 text-muted-foreground">{t.nodes.subtitle}</p>
        </div>
            ...
            {addMenuOpen && (
              <>
                ...
                <div className="glass-panel-strong absolute end-0 z-20 mt-2 w-64 rounded-xl p-1.5 animate-fade-in-up">
                  ...
                  >
                    <span className="text-sm font-medium text-foreground">Add Node via SSH</span>
                  ...
                  >
                    ...
                    <span className="text-xs text-muted-foreground">The node installs itself and registers with the panel</span>
                  </button>
                </div>
              </>
            )}
      ...
      {loading ? (
        ...
        <EmptyState
          icon={<Server size={24} />}
          title="No Iran nodes yet"
          description="Add a node via SSH or register one using the CA certificate flow to get started."
          action={<Button onClick={() => setShowSSHWizard(true)} icon={<Plus size={16} />}>{t.nodes.addNode}</Button>}
        />
      ) : (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {nodes.map((node) => (
      ...
      {/* Register via CA certificate (existing, working flow) */}
      <Modal
      ...
      <AddNodeSSHWizard
        open={showSSHWizard}
        role="iran"
        onClose={() => setShowSSHWizard(false)}
        onCompleted={refresh}
      />
      ...
      >
        <div className="mb-4 rounded-xl bg-secondary/50 p-3 text-sm text-muted-foreground">
          <strong className="text-foreground">Node installation:</strong> copy this certificate — you'll be asked to paste
          it during node installation.
        </div>
        {certLoading ? (
...
const RegisterNodeForm = ({ onSuccess, onCancel }: { onSuccess: () => void; onCancel: () => void }) => {
  const { show } = useToast()
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true)
    try {
      await registerNode({ name, ip_address: ipAddress, api_port: parseInt(apiPort) || 8888 })
      show('Node registered', 'success')
      onSuccess()
    } catch (err: any) {
      show(err?.response?.data?.detail || 'Failed to add node', 'error')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="flex justify-end gap-3 pt-2">
        ...
        <Button type="submit" loading={submitting}>Register Node</Button>
      </div>
    </form>
  )
}
