#!/bin/bash
# Nosrat Node Installer

set -e

echo "=== Nosrat Node Installer ==="
echo ""
...
if [ -d "$INSTALL_DIR" ] && [ -f "$INSTALL_DIR/docker-compose.yml" ]; then
    echo "Nosrat Node already installed in $INSTALL_DIR"
    cd "$INSTALL_DIR"
else
    echo "Setting up Nosrat Node..."
    mkdir -p "$INSTALL_DIR"
    cd "$INSTALL_DIR"
...
if [ ! -f "Dockerfile" ]; then
    ...
    TEMP_DIR=$(mktemp -d)
    git clone --depth 1 $GIT_BRANCH https://github.com/zZedix/Nosrat.git "$TEMP_DIR" || {
...
# Install CLI
if [ -f "/opt/nosrat/cli/nosrat-node.py" ]; then
    sudo cp /opt/nosrat/cli/nosrat-node.py /usr/local/bin/nosrat-node
    sudo chmod +x /usr/local/bin/nosrat-node
elif [ -f "$INSTALL_DIR/../Nosrat/cli/nosrat-node.py" ]; then
    sudo cp "$INSTALL_DIR/../Nosrat/cli/nosrat-node.py" /usr/local/bin/nosrat-node
    sudo chmod +x /usr/local/bin/nosrat-node
else
    ...
    sudo curl -L https://raw.githubusercontent.com/zZedix/Nosrat/${CLI_BRANCH}/cli/nosrat-node.py -o /usr/local/bin/nosrat-node
    sudo chmod +x /usr/local/bin/nosrat-node
fi
...
if docker pull ghcr.io/zzedix/nosrat-node:${SMITE_VERSION} 2>/dev/null; then
...
# Check status
if docker ps | grep -q nosrat-node; then
    echo ""
    echo "✅ Nosrat Node installed successfully!"
    echo ""
    echo "Node API: http://localhost:$NODE_API_PORT"
    echo ""
    echo "Check status with: nosrat-node status"
    echo ""
else
